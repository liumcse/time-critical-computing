...
struct RBTree OS_REC_TREE;
// struct RBNode REC_TASK_ARR[5];

#ifdef VSC_INCLUDE_SOURCE_FILE_NAMES
const CPU_CHAR *os_task__c = "$Id: $";
#endif

int rbtree_key_cmp(OS_TCB *a, OS_TCB *b)
{
                      ...
                      OS_ERR *p_err)
{
    ...
    { /* ---------- CANNOT CREATE A TASK FROM AN ISR ---------- */
    ...
    { /* Cannot set to Idle Task priority                       */
        *p_err = OS_ERR_PRIO_INVALID;
        return;
    }

    if (p_tcb == (OS_TCB *)0)
    { /* See if want to change priority of 'self'               */
        CPU_CRITICAL_ENTER();
        p_tcb = OSTCBCurPtr;
        CPU_CRITICAL_EXIT();
        self = DEF_TRUE;
    ...
    case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:
        switch (p_tcb->PendOn)
        {                            /* What to do depends on what we are pending on           */
...
*              p_task         is a pointer to the task's code
*
*              p_period       is a number of the task's period
*
*              p_arg          is a pointer to an optional data area which can be used to pass parameters to
*                             the task when the task first executes.  Where the task is concerned it thinks
*                             it was invoked and passed the argument 'p_arg' as follows:
...
void OSRecTaskCreate(OS_TCB *p_tcb,
                     ...
                     OS_ERR *p_err)
{
    // TODO(liumcse): declare Red Black Tree node
    ...
    if (period == (CPU_INT32U)0)
    { /* User must supply a valid task period                   */
        *p_err = OS_ERR_TASK_INVALID;
        return;
    }
    if (p_stk_base == (CPU_STK *)0)
    ...
    *p_err = OS_ERR_NONE;
    ...
    // TODO(liumcse): why OS_CFG_PRIO_MAX and what does the formula mean?
    p_tcb->Prio = (period / 1000) * (OS_CFG_PRIO_MAX - 4) / 120 + 4; /* Save the task's priority using Rate Monotonic (RM) scheduling                              */
    ...
    struct RBNode node;
    node.value = p_tcb;
    /* Add this node to red black tree */
    rbtree_insert(&OS_REC_TREE, &node);
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
    if (p_tcb == &OSIntQTaskTCB)
    { /* Cannot delete the ISR handler task                     */
...
*
*              R               OS_ERR_NONE            The call was successful and the message was sent
                         ...
                         OS_ERR *p_err)
{
    ...
    { /* See if task already been signaled                      */
        OSTCBCurPtr->SemCtr--;
                  ...
                  OS_ERR *p_err)
{
    ...
    if ((p_tcb->Opt & OS_OPT_TASK_STK_CHK) == (OS_OPT)0)
                   ...
                   OS_ERR *p_err)
{
    ...
    if (p_tcb == &OSIdleTaskTCB)
    { /* Make sure not suspending the idle task                 */
...
#if OS_CFG_ISR_POST_DEFERRED_EN > 0u
    if (p_tcb == &OSIntQTaskTCB)
    { /* Not allowed to suspend the ISR handler task            */
    ...
    CPU_CRITICAL_ENTER();
    if (p_tcb == (OS_TCB *)0)
    { /* See if specified to suspend self                       */
        p_tcb = OSTCBCurPtr;
    }
    ...
    {
        if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0)
        { /* Can't suspend when the scheduler is locked             */
            CPU_CRITICAL_EXIT();
            *p_err = OS_ERR_SCHED_LOCKED;
            return;
        }
